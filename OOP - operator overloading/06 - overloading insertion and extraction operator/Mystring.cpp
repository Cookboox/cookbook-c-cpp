...
std::istream& operator>>(std::istream& is, Mystring& rhs)
{
	...
	//std::cout << "size of \0 is " << sizeof('\0') << " byte\n";
	//std::cout << "size of buffer is " << std::strlen(buffer) << "\n"; // !!
	if(std::strlen(buffer) < std::strlen(rhs.str))
	{
	...
	{
		is >> buffer;
		rhs = Mystring{ buffer }; // move constructor will be used here
		delete[] buffer;
		return is;
	}	
}